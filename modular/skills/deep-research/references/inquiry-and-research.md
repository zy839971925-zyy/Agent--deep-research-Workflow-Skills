...
**Related:** problem framing and effort (route to the relevant sibling Skill if needed), [retrieval and observation](retrieval-and-observation.md), [evidence and provenance](evidence-and-provenance.md), relationships and systems (route to the relevant sibling Skill if needed), hypotheses and bias control (route to the relevant sibling Skill if needed).
...
Materialize it into durable state only when the runtime/task justifies persistence. For state semantics, use state model and invariants (route to the relevant sibling Skill if needed).
